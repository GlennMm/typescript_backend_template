...
export const subscriptionPlans = sqliteTable('subscription_plans', {
  ...
  name: text('name', { enum: ['Free', 'Pro', 'Enterprise'] }).notNull().unique(),
...
export const tenants = sqliteTable('tenants', {
  ...
  // Subscription
  subscriptionPlanId: text('subscription_plan_id').notNull().references(() => subscriptionPlans.id),
  subscriptionStatus: text('subscription_status', {
    enum: ['active', 'inactive', 'suspended', 'grace_period']
  }).notNull().default('active'),
  subscriptionStartDate: integer('subscription_start_date', { mode: 'timestamp' }).notNull(),
...
export const superAdminRefreshTokens = sqliteTable('super_admin_refresh_tokens', {
  id: text('id').primaryKey(),
